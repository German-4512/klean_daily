...
function renderLineChart(container, labels, series) {
  ...
  series.forEach((item) => {
    item.values.forEach((value, index) => {
      ...
      label.setAttribute('y', Math.max(y - 8, 12));
  ...
  series.forEach((item) => {
    ...
    polyline.classList.add('line-path');
    if (item.className) {
      polyline.classList.add(item.className);
    }
  ...
  labelsRow.className = 'line-labels';
  labelsRow.innerHTML = labels.map((label) => `<span>${label}</span>`).join('');
...
function buildTipoVentaSeries(data, year) {
  const seguimiento = Array.from({ length: 12 }, () => 0);
  const primeraVez = Array.from({ length: 12 }, () => 0);
  let hasData = false;
  ...
  data.forEach((item) => {
    ...
    const tipoVenta = normalizeValue(item.tipo_de_venta || '');
    const cantidad = Number(confirmada.cantidad) || 0;
    if (!tipoVenta || cantidad <= 0) return;
    hasData = true;

    if (tipoVenta === normalizeValue('consulta seguimiento mascotas')) {
      seguimiento[fecha.getMonth()] += cantidad;
    } else if (tipoVenta === normalizeValue('consulta primera vez mascotas')) {
      primeraVez[fecha.getMonth()] += cantidad;
    }
  });
  ...
  return {
    hasData,
    series: [
      { name: 'Consulta seguimiento Mascotas', values: seguimiento, className: 'vets' },
      { name: 'Consulta Primera Vez Mascotas', values: primeraVez, className: 'preventas' }
    ]
  };
}

async function loadMetrics(context, rangeOptions = {}) {
  ...
  try {
    ...
    const scopedVentas = context.isAdmin ? ventasData : ventasData.filter((item) => matchesVet(item.veterinario, selectedVet));
    const scopedCreditos = context.isAdmin ? (creditosResponse.ok ? creditosData : []) : (creditosResponse.ok ? creditosData.filter((item) => matchesVet(item.veterinario, selectedVet)) : []);
    ...
    const metrics = computeMetrics(scopedVentas, scopedCreditos, recaudoMapMetrics, metricsRange, selectedVet);
    ...
    const daySeries = buildDaySeries(scopedVentas, range, selectedVet);
    ...
    const montoMonthSeries = buildMontoMonthSeries(scopedVentas, recaudosData, range, selectedVet);
    ...
    const tipoVentaChart = document.getElementById('chart-tipo-venta');
    const tipoVentaSeries = buildTipoVentaSeries(scopedVentas, range.start.getFullYear());
    const lineData = tipoVentaSeries.hasData
      ? tipoVentaSeries.series
      : tipoVentaSeries.series.map((item) => ({ ...item, values: item.values.map(() => 0) }));
    renderLineChart(tipoVentaChart, monthLabels, lineData);

    const periodSubtitle = document.getElementById('period-subtitle');
    if (context.isAdmin) {
      periodSubtitle.textContent = `Vista administrador - ${periodLabel}`;
    } else if (vetName) {
      periodSubtitle.textContent = `Veterinario: ${vetName} - ${periodLabel}`;
    } else {
      periodSubtitle.textContent = `Veterinario: sin asignar - ${periodLabel}`;
    }
    ...
    document.getElementById('chart-tipo-venta-year').textContent = range.start.getFullYear();
  } catch (error) {
    console.error('Error al cargar mis ventas vets:', error);
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  btnApply.addEventListener('click', () => {
    ...
    loadMetrics(context, { month: monthValue });
  });
